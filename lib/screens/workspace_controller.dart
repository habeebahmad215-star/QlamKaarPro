...
class WorkspaceController extends ChangeNotifier {
  ...
  set elements(List<DesignElement> val) { pages[currentPageIndex].elements = val; notifyListeners(); }

  Color get pageColor => pages[currentPageIndex].pageColor;
  set pageColor(Color val) { pages[currentPageIndex].pageColor = val; notifyListeners(); }

  List<Color>? get bgGradient => pages[currentPageIndex].bgGradient;
  set bgGradient(List<Color>? val) { pages[currentPageIndex].bgGradient = val; notifyListeners(); }

  double get canvasRatio => pages[currentPageIndex].canvasRatio;
  set canvasRatio(double val) { pages[currentPageIndex].canvasRatio = val; notifyListeners(); }

  Uint8List? get bgImageBytes => pages[currentPageIndex].bgImageBytes;
  set bgImageBytes(Uint8List? val) { pages[currentPageIndex].bgImageBytes = val; notifyListeners(); }

  DesignElement? get selectedElement {
    if (selectedId == null) return null;
    try { return elements.firstWhere((e) => e.id == selectedId); } catch (e) { return null; }
  }
  ...
  Map<String, Map<int, Map<String, dynamic>>> deepCopyMultiStyles(Map<String, Map<int, Map<String, dynamic>>> source) {
    ...
    source.forEach((key, val) {
      copy[key] = {};
      val.forEach((idx, styleMap) { copy[key]![idx] = Map<String, dynamic>.from(styleMap); });
    });
    return copy;
  }
  ...
  void saveState() {
    ...
    redoStack.clear(); redoMultiStylesStack.clear();
    if (undoStack.length > 10) { undoStack.removeAt(0); undoMultiStylesStack.removeAt(0); }
  }
  ...
  void undoAction() {
    if (undoStack.isNotEmpty) {
      ...
      selectedId = null; activeToolbarMenu = 'main'; 
      HapticFeedback.lightImpact(); notifyListeners();
    }
  }
  ...
  void rotateElement(DragUpdateDetails d, DesignElement e) { e.angle += (d.delta.dx + d.delta.dy) * 0.015; notifyListeners(); }

  void deleteSelected() {
    if (selectedId != null) { saveState(); elements.removeWhere((e) => e.id == selectedId); textMultiStyles.remove(selectedId); selectedId = null; activeToolbarMenu = 'main'; notifyListeners(); }
  }
  ...
  void duplicateSelected() {
    if (selectedId != null) {
      ...
      if (textMultiStyles.containsKey(sel.id)) {
        ...
        textMultiStyles[sel.id]!.forEach((idx, styleMap) { textMultiStyles[newEl.id]![idx] = Map<String, dynamic>.from(styleMap); });
      }
      elements.add(newEl); selectedId = newEl.id; activeToolbarMenu = 'main'; notifyListeners();
    }
  }
  ...
  void bringForward() {
    ...
    if (idx < elements.length - 1) { var item = elements.removeAt(idx); elements.insert(idx + 1, item); notifyListeners(); }
  }

  void sendBackward() {
    if (selectedId == null) return;
    saveState(); int idx = elements.indexWhere((e) => e.id == selectedId);
    if (idx > 0) { var item = elements.removeAt(idx); elements.insert(idx - 1, item); notifyListeners(); }
  }

  Future<void> captureAndSave(BuildContext context, String format) async {
    selectedId = null; isExporting = true; activeToolbarMenu = 'main'; notifyListeners();
    ...
    try {
      ...
      if (format == 'JPG' || format == 'PNG') {
        ...
        if (context.mounted && result != null && result['isSuccess'] == true) { _showSuccessDialog(context, 'Saved to Gallery!', 'Aapka $format design gallery mein save ho gaya hai.'); }
      } else if (format == 'PDF') {
    ...
    } catch (e) { debugPrint('Export Error: $e'); } finally { isExporting = false; notifyListeners(); }
  }
