...
class _ProWorkspaceScreenState extends State<ProWorkspaceScreen> {
  ...
  // YAHAN SE FAST UPDATE HOGA (NO LAG)
  ...
  void _resizeEdge(DragUpdateDetails d, String edge, DesignElement e) {
    ...
    if (edge == 'R') {
      e.width = max(80.0, e.width + ldx); // Minimum 80 width so text never hides
    } else if (edge == 'L') {
      double oldW = e.width; 
            ...
            child: Column(
              children: [
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    const Text('Urdu Library شاعری / اقوال', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF8B5CF6))),
                    IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(context))
                  ]
                ),
                TabBar(
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return Card(
                            ...
                            child: ListTile(
                              title: Text(text, textDirection: TextDirection.rtl, textAlign: TextAlign.center, style: const TextStyle(fontFamily: 'JameelNoori', fontSize: 20, height: 1.5)),
          ...
          child: Column(
            ...
            children: [
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  const Text('Tashkeel & Symbols اعراب', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF8B5CF6))),
                  IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(context))
                ]
              ),
              const Divider(),
                  ...
                  itemBuilder: (context, index) {
                    return InkWell(
                      ...
                      child: Container(
                        ...
                        child: Text(tashkeelList[index], style: const TextStyle(fontSize: 22, fontFamily: 'JameelNoori', color: Colors.black87))
                      )
                    );
                  }
                )
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        const SizedBox(width: 15),
                        Expanded(flex: 2, child: ElevatedButton.icon(style: ElevatedButton.styleFrom(backgroundColor: isRTL ? const Color(0xFF10B981) : const Color(0xFF6366F1), padding: const EdgeInsets.symmetric(vertical: 15), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), elevation: 4), 
                        onPressed: () { 
                          if (controller.text.isNotEmpty) { 
                            saveState(); 
                            
                            // PERFECT TEXT WIDTH LOGIC
                            ...
                            if (existingElement != null) { 
                              ...
                              var newEl = DesignElement(id: Random().nextInt(10000).toString(), x: 40, y: 100, content: controller.text, width: calcW); 
                ...
                child: Column(
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ElevatedButton.icon(
                          onPressed: () { setModalState(() { List<String> newRow = List.generate(tempTable[0].length, (i) => 'New Data'); tempTable.add(newRow); }); },
                          icon: const Icon(Icons.table_rows),
                          label: const Text('+ Add Row')
                        ),
                        ElevatedButton.icon(
                          onPressed: () { setModalState(() { for(int i=0; i<tempTable.length; i++) { tempTable[i].add('New Col'); } }); },
                        ...
                        children: [
                          Row(
                            children: [
                              ...
                              Expanded(child: Slider(value: sel.width, min: 100.0, max: 800.0, activeColor: const Color(0xFF8B5CF6), onChanged: (val) { setModalState((){ sel.width = val; }); _triggerCanvasUpdate(); }))
                            ]
                          ),
                          Row(
                            children: [
                              const Text('Total Height: ', style: TextStyle(fontWeight: FontWeight.bold)),
                              Expanded(child: Slider(value: sel.height, min: 50.0, max: 800.0, activeColor: Colors.blue, onChanged: (val) { setModalState((){ sel.height = val; }); _triggerCanvasUpdate(); }))
                            ]
                          ),
                        ],
                      ),
  ...
  // FIXED TEXT ALIGNMENT THAT WORKS ON CANVAS
  void _toggleAlignment(DesignElement sel) {
    ...
    _triggerCanvasUpdate(); // Refreshes Canvas Immediately
  }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: Row(
                        children: [
                          Expanded(child: TextField(controller: hexCtrl, decoration: const InputDecoration(hintText: 'Hex Code: #FF0000', border: OutlineInputBorder(), contentPadding: EdgeInsets.symmetric(horizontal: 10)))),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: Row(
                        children: [
                          Expanded(child: TextField(controller: hexCtrl, decoration: const InputDecoration(hintText: 'Custom Hex: #FF0000', border: OutlineInputBorder(), contentPadding: EdgeInsets.symmetric(horizontal: 10)))),
                          const SizedBox(width: 10),
                          ElevatedButton(
                            style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF8B5CF6), padding: const EdgeInsets.symmetric(vertical: 12)),
                            onPressed: () {
  ...
  Widget _buildShapeOption(DesignElement sel, StateSetter setModalState, String title, int val, IconData icon) {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: isSel ? const Color(0xFF8B5CF6).withOpacity(0.1) : Colors.transparent, borderRadius: BorderRadius.circular(10), border: Border.all(color: isSel ? const Color(0xFF8B5CF6) : Colors.grey.shade300)),
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(icon, color: isSel ? const Color(0xFF8B5CF6) : Colors.grey.shade600, size: 30),
            const SizedBox(height: 5),
            Text(title, style: TextStyle(fontSize: 12, fontWeight: isSel ? FontWeight.bold : FontWeight.normal, color: isSel ? const Color(0xFF8B5CF6) : Colors.black87))
          ]
        )
      )
    );
              ...
              child: Column(
                children: [
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      const Text('3D Perspective تھری ڈی زاویہ', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                      IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(context))
                    ]
                  ),
                  const SizedBox(height: 10),
              ...
              child: Column(
                children: [
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      const Text('Nudge Tool خردبینی حرکت', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                      IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(context))
                    ]
                  ),
                  const SizedBox(height: 10),
                  Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [IconButton(icon: const Icon(Icons.arrow_upward, size: 35, color: const Color(0xFF8B5CF6)), onPressed: () => move(0, -2))]
                  ),
                  Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      IconButton(icon: const Icon(Icons.arrow_back, size: 35, color: const Color(0xFF8B5CF6)), onPressed: () => move(-2, 0)),
                      const SizedBox(width: 40),
                      IconButton(icon: const Icon(Icons.arrow_forward, size: 35, color: const Color(0xFF8B5CF6)), onPressed: () => move(2, 0))
                    ]
                  ),
                  Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [IconButton(icon: const Icon(Icons.arrow_downward, size: 35, color: const Color(0xFF8B5CF6)), onPressed: () => move(0, 2))]
                  ),
                ]
              )
            );
  ...
  Widget _buildPillHandle(bool isHorizontal, Function(DragUpdateDetails) onDrag) {
    return GestureDetector(
      ...
      onPanUpdate: onDrag,
      child: Container(
        padding: const EdgeInsets.all(10), // Invisible touch target extension
                                  ...
                                  child: Stack(
                                    ...
                                    children: [
                                      ...
                                      ...elements.map((e) {
                                        ...
                                        // 100% PRO SELECTION BOX WITH EXTENDED HIT AREAS
                                        return Positioned(
                                          left: e.x - 10, // Account for invisible handle padding
                                          top: e.y - 10,
                                          child: Transform(
                                            transform: matrix, alignment: Alignment.center,
                                            child: SizedBox(
                                              width: currentWidth + 20, height: currentHeight + 20,
                                              child: Stack(
                                                clipBehavior: Clip.none,
                                                children: [
                                                  ...
                                                  if (isSel) ...[
                                                    // Edge Pills
                                                    Positioned(top: 10 - 14, left: 10 + currentWidth/2 - 22, child: _buildPillHandle(true, (d) => _resizeEdge(d, 'T', e))),
                                                    Positioned(bottom: 10 - 14, left: 10 + currentWidth/2 - 22, child: _buildPillHandle(true, (d) => _resizeEdge(d, 'B', e))),
                                                    Positioned(left: 10 - 14, top: 10 + currentHeight/2 - 22, child: _buildPillHandle(false, (d) => _resizeEdge(d, 'L', e))),
                                                    Positioned(right: 10 - 14, top: 10 + currentHeight/2 - 22, child: _buildPillHandle(false, (d) => _resizeEdge(d, 'R', e))),

                                                    // Corner Circles
                                                    Positioned(top: 10 - 16, left: 10 - 16, child: _buildCircleHandle((d) => _scaleCorner(d, e))),
                                                    Positioned(top: 10 - 16, right: 10 - 16, child: _buildCircleHandle((d) => _scaleCorner(d, e))),
                                                    Positioned(bottom: 10 - 16, left: 10 - 16, child: _buildCircleHandle((d) => _scaleCorner(d, e))),
                                                    Positioned(bottom: 10 - 16, right: 10 - 16, child: _buildCircleHandle((d) => _scaleCorner(d, e))),

                                                    // Rotate Handle (Top Right)
                                                    Positioned(
                                                      top: 10 - 45, right: 10 - 45, 
                                                      child: _buildIconHandle(Icons.rotate_right, (d) => _rotateHandle(d, e))
                                                    ),
                                                    
                                                    // Scale Handle (Bottom Left)
                                                    Positioned(
                                                      bottom: 10 - 45, left: 10 - 45, 
