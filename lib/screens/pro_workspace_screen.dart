...
class ProWorkspaceScreen extends StatefulWidget {
  final ProjectModel? project; final String? initialAction;
...
class _ProWorkspaceScreenState extends State<ProWorkspaceScreen> {
  ...
  void initState() {
    super.initState();
    if (widget.project != null) { projectId = widget.project!.id; projectName = widget.project!.name; pages = widget.project!.pages; } 
    else { projectId = DateTime.now().millisecondsSinceEpoch.toString(); projectName = 'Design_$projectId'; pages = [DesignPage(title: 'Page 1', elements: [], pageColor: Colors.white)]; }
    if (widget.initialAction != null) { WidgetsBinding.instance.addPostFrameCallback((_) { if (widget.initialAction == 'text_editor') _showTextComposerDialog(); else if (widget.initialAction == 'images') addImageFromGallery(fromModal: false); else if (widget.initialAction == 'elements') showAddNewModal(); }); }
  }
  ...
  double _getElWidth(DesignElement e) => max(e.width, 80.0);
  ...
  Future<void> _saveProjectLocally() async {
    final prefs = await SharedPreferences.getInstance(); List<String> savedStrings = prefs.getStringList('qalamkaar_projects') ?? [];
    ProjectModel p = ProjectModel(id: projectId, name: projectName, pages: pages, lastModified: DateTime.now().millisecondsSinceEpoch);
    savedStrings.removeWhere((str) => jsonDecode(str)['id'] == projectId); savedStrings.add(jsonEncode(p.toJson()));
    await prefs.setStringList('qalamkaar_projects', savedStrings); HapticFeedback.lightImpact(); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Project Saved Successfully!')));
  }
  ...
  void undoAction() { if (undoStack.isNotEmpty) { redoStack.add(elements.map((e) => e.clone()).toList()); setState(() { elements = undoStack.removeLast(); selectedId = null; }); _triggerCanvasUpdate(); } }
  void redoAction() { if (redoStack.isNotEmpty) { undoStack.add(elements.map((e) => e.clone()).toList()); setState(() { elements = redoStack.removeLast(); selectedId = null; }); _triggerCanvasUpdate(); } }

  void _resizeEdge(DragUpdateDetails d, String edge, DesignElement e) {
    setState(() {
      double ldx = d.delta.dx, ldy = d.delta.dy;
      if (e.angle != 0) { double cosA = cos(-e.angle), sinA = sin(-e.angle); ldx = d.delta.dx * cosA - d.delta.dy * sinA; ldy = d.delta.dx * sinA + d.delta.dy * cosA; }
      if (edge == 'R') { e.width = max(80.0, e.width + ldx); } 
      else if (edge == 'L') { double oldW = e.width; e.width = max(80.0, e.width - ldx); if (e.width > 80.0) { e.x += (oldW - e.width) * cos(e.angle); e.y += (oldW - e.width) * sin(e.angle); } } 
      else if (edge == 'B') { if(!e.isText) e.height = max(30.0, e.height + ldy); } 
      else if (edge == 'T') { if(!e.isText) { double oldH = e.height; e.height = max(30.0, e.height - ldy); if (e.height > 30.0) { e.x -= (oldH - e.height) * sin(e.angle); e.y += (oldH - e.height) * cos(e.angle); } } }
    });
  ...
  void _scaleCorner(DragUpdateDetails d, DesignElement e) {
    setState(() {
      ...
      if (e.width + delta > 80) {
        double ratio = e.width / (e.height > 0 ? e.height : 1); e.width += delta; 
        if (!e.isText) e.height += delta / ratio; if (e.isText) e.fontSize = max(10.0, e.fontSize + delta * 0.2);
  ...
  void _rotateHandle(DragUpdateDetails d, DesignElement e) { setState(() { e.angle += (d.delta.dx + d.delta.dy) * 0.01; }); _triggerCanvasUpdate(); }
  ...
  Future<void> _captureAndSave(String format) async {
    ...
    try {
      ...
      ByteData? byteData = await image.toByteData(format: ui.ImageByteFormat.png); Uint8List pngBytes = byteData!.buffer.asUint8List();
      if (format == 'JPG' || format == 'PNG') {
        ...
        if (mounted && result != null && result['isSuccess'] == true) _showSuccessDialog('Saved!', 'Aapka $format design gallery mein save ho gaya hai.');
      } else if (format == 'PDF') {
        ...
        Uint8List pdfBytes = await pdf.save(); await Printing.sharePdf(bytes: pdfBytes, filename: "QalamKaarPro_${DateTime.now().millisecondsSinceEpoch}.pdf");
      }
  ...
  void _addTable() { saveState(); setState(() { elements.add(DesignElement(id: Random().nextInt(10000).toString(), x: 40, y: 100, content: 'Table', width: 300, height: 150, isText: false, isTable: true, tableData: [['Col 1', 'Col 2'], ['Data 1', 'Data 2']])); selectedId = elements.last.id; }); _triggerCanvasUpdate(); }
  ...
  Future<void> _setCanvasBackground() async { try { final XFile? image = await _picker.pickImage(source: ImageSource.gallery); if (image != null) { final bytes = await image.readAsBytes(); setState(() { bgImageBytes = bytes; bgGradient = null; pageColor = Colors.white; }); _triggerCanvasUpdate(); } } catch (e) {} }
