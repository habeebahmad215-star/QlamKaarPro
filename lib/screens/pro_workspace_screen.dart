...
class _ProWorkspaceScreenState extends State<ProWorkspaceScreen> with WorkspaceModals {
  ...
  void initState() {
    ...
    // ⏱️ Start Auto-Save Timer (Every 1 Minute)
  ...
  @override
  void dispose() {
    _autoSaveTimer?.cancel(); // ❌ Cancel Timer on exit
  ...
  // 💾 Modified Save logic to handle silent Auto-Saves
  Future<void> _saveProjectLocally({bool isAutoSave = false}) async {
    try {
      ...
      // Sirf manual save par snackbar dikhaye, Auto-Save par chupchap save kare
      if (!isAutoSave) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          // 🔥 Save button removed. Only Export is here!
          Center(
            child: InkWell(
