...
class _ProWorkspaceScreenState extends State<ProWorkspaceScreen> {
  ...
  void _showTashkeelModal(TextEditingController controller) {
    final List<String> tashkeelList = ['َ', 'ِ', 'ُ', 'ً', 'ٍ', 'ٌ', 'ّ', 'ْ', 'ٓ', 'ٰ', 'ٖ', 'ٗ', 'ۖ', 'ۗ', 'ۘ', 'ۙ', 'ۚ', 'ۛ', 'ۜ', '۝', '۞', '۩', 'ﷲ', 'ﷺ', 'ؓ', 'ؒ', 'ﷻ', 'ؐ', 'ؑ', '﷽', 'سبحان الله', 'الحمد لله', 'لا إله إلا الله', 'الله أكبر', 'جل جلاله', 'بسم الله'];
              ...
              child: Container(
                ...
                child: Column(
                  children: [
                    ...
                    Expanded(
                      child: Container(
                        padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.grey.shade50, border: Border.all(color: isRTL ? const Color(0xFF10B981).withOpacity(0.3) : const Color(0xFF6366F1).withOpacity(0.3), width: 1.5), borderRadius: BorderRadius.circular(16)),
          ...
          builder: (BuildContext context, StateSetter setModalState) {
            return Padding(
              ...
              child: Container(
                ...
                child: Column(
                  children: [
                    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('Edit Table ٹیبل ایڈٹ کریں', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF8B5CF6))), IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(context))]),
                                ...
                                children: [
                                  ...row.asMap().entries.map((colEntry) {
                                    ...
                                    return Container(
                                      ...
                                      child: TextField(controller: TextEditingController(text: tempTable[rowIndex][colIndex]), textDirection: TextDirection.rtl, style: const TextStyle(fontFamily: 'JameelNoori'), decoration: const InputDecoration(border: OutlineInputBorder(), contentPadding: EdgeInsets.symmetric(horizontal: 8, vertical: 8)), onChanged: (val) { tempTable[rowIndex][colIndex] = val; })
                                    );
                                  }).toList(),
                                  if (tempTable.length > 1)
                                    IconButton(icon: const Icon(Icons.delete, color: Colors.red, size: 20), onPressed: () { setModalState(() { tempTable.removeAt(rowIndex); }); })
                                ],
                              );
                            }).toList(),
                          ),
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          Row(children: [const Text('Total Width: ', style: TextStyle(fontWeight: FontWeight.bold)), Expanded(child: Slider(value: sel.width, min: 100.0, max: 800.0, activeColor: const Color(0xFF8B5CF6), onChanged: (val) { setModalState((){ sel.width = val; }); _triggerCanvasUpdate(); }))]),
                          Row(children: [const Text('Total Height: ', style: TextStyle(fontWeight: FontWeight.bold)), Expanded(child: Slider(value: sel.height, min: 50.0, max: 800.0, activeColor: Colors.blue, onChanged: (val) { setModalState((){ sel.height = val; }); _triggerCanvasUpdate(); }))]),
                        ],
                      ),
                    ),
                    const SizedBox(height: 15),
            ...
            Expanded(child: ListView(
              ...
              children: [
                const Padding(
                  ...
                  child: Text('Pre-installed Premium Fonts', style: TextStyle(fontWeight: FontWeight.w900, color: Colors.grey)),
                ),
                          ...
                          children: [
                            Expanded(
                              ...
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text(font['name']!, style: TextStyle(fontWeight: FontWeight.bold, color: isSelected ? const Color(0xFF8B5CF6) : Colors.black87)),
                ...
                if (customFonts.isNotEmpty) ...[
                  const Padding(
                    ...
                    child: Text('My Custom Fonts', style: TextStyle(fontWeight: FontWeight.w900, color: Colors.grey)),
                  ),
                            ...
                            children: [
                              Expanded(
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text(fontName, style: TextStyle(fontWeight: FontWeight.bold, color: isSelected ? const Color(0xFF8B5CF6) : Colors.black87)),
                                                              ...
                                                              child: Row(
                                                                ...
                                                                children: [
                                                                  GestureDetector(
                                                                    ...
                                                                    child: const Icon(Icons.open_with, size: 20, color: Colors.black54)
                                                                  ),
                                                                  const SizedBox(width: 16),
                                                                  InkWell(onTap: duplicateSelected, child: const Icon(Icons.copy, size: 20, color: Colors.black54)),
                                                                  const SizedBox(width: 16),
                                                                  InkWell(onTap: deleteSelected, child: const Icon(Icons.delete_outline, size: 20, color: Colors.redAccent)),
                                                                ]
                                                              )
                                                            )
                                                          )
