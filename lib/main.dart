...
class _ProWorkspaceScreenState extends State<ProWorkspaceScreen> {
  ...
  // ==== EXPORT & SAVE LOGIC (🔥 4K ULTRA HD FIX 🔥) ====
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 10),
            _buildExportOption(Icons.picture_as_pdf, 'Save as HD PDF', '4K Quality for Crisp Printing', Colors.red, () {
  ...
  Future<void> _captureAndSave(String format) async {
    ...
    // UI ko theek se hide hone ke liye 600ms ka waqt diya
    await Future.delayed(const Duration(milliseconds: 600));
    
    try {
      RenderRepaintBoundary boundary = _canvasKey.currentContext!.findRenderObject() as RenderRepaintBoundary;
      
      // 🚀 THE MAGIC: 6.0 Zoom ka matlab hai ek normal canvas ab 4000+ pixels ka ho jayega!
      ...
      if (format == 'JPG' || format == 'PNG') {
        ...
        if (mounted && result != null && result['isSuccess'] == true) {
          _showSuccessDialog('Saved to Gallery!', 'Aapka 4K High-Res $format design gallery mein save ho gaya hai.');
        }
      } 
      else if (format == 'PDF') {
        ...
        pdf.addPage(
          pw.Page(
            pageFormat: PdfPageFormat(imgWidth, imgHeight), // 100% Fit
            margin: pw.EdgeInsets.zero, 
            build: (pw.Context context) {
              return pw.Image(imagePdf, fit: pw.BoxFit.contain); // Image ko bilkul theek fit karega
            },
          ),
        );
        ...
        await Printing.sharePdf(
          bytes: pdfBytes, 
          filename: "QalamKaarPro_HD_${DateTime.now().millisecondsSinceEpoch}.pdf"
        );
      }
    } catch (e) {
      debugPrint('Export Error: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Error: Shayad file size bohot bada ho gaya hai. Dobara koshish karein.')));
      }
    } finally {
      setState(() {
        _isExporting = false; 
