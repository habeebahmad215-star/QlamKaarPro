...
class DesignPage {
  String title; List<DesignElement> elements; 
  Color pageColor; 
  Uint8List? bgImageBytes; 
  double canvasRatio; 
  List<Color>? bgGradient; // 🔥 NEW: Support for Gradient Backgrounds
...
class DesignElement {
  ...
  int clipShape; 
  bool isTinted; 
  Color? customGradColor1; Color? customGradColor2; 

  double cornerRadius; 
  Uint8List? textTextureBytes; 
...
class _ProWorkspaceScreenState extends State<ProWorkspaceScreen> {
  ...
  Future<void> _setCanvasBackground() async { 
    try { 
      ...
      if (image != null) { 
        final bytes = await image.readAsBytes(); 
        setState(() { bgImageBytes = bytes; bgGradient = null; }); // Reset gradient when image is set
      } 
    } catch (e) { 
  ...
  void _pickCustomGradColor(DesignElement sel, int colorNum, StateSetter parentSetState) {
    showModalBottomSheet(context: context, builder: (ctx) => Container(height: 300, padding: const EdgeInsets.all(20), child: Column(children: [
      const Text('Pick a Color', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)), const SizedBox(height: 10),
      Expanded(child: GridView.builder(gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 6, crossAxisSpacing: 10, mainAxisSpacing: 10), itemCount: _proColorPalette.length, itemBuilder: (ctx, index) { 
        return InkWell(onTap: () { 
          if(colorNum == 1) sel.customGradColor1 = _proColorPalette[index]; 
          else sel.customGradColor2 = _proColorPalette[index]; 
          parentSetState((){}); Navigator.pop(ctx); 
        }, child: Container(decoration: BoxDecoration(color: _proColorPalette[index], shape: BoxShape.circle, border: Border.all(color: Colors.black26)))); 
      }))
    ])));
  }

  // 🔥 NEW PRO FEATURE: CANVAS BACKGROUND COLOR MODAL 🔥
  void _showCanvasBgColorModal() { 
    ...
    showModalBottomSheet(context: context, isScrollControlled: true, backgroundColor: Colors.white, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (context) { 
      return StatefulBuilder(builder: (context, setModalState) { 
        return Padding(padding: EdgeInsets.only(bottom: MediaQuery.of(context).viewInsets.bottom), child: Container(height: 480, padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
  ...
  // 🔥 NEW PRO FEATURE: CANVAS BACKGROUND GRADIENT MODAL 🔥
    ...
    showModalBottomSheet(context: context, backgroundColor: Colors.white, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (context) { 
      return StatefulBuilder(builder: (context, setModalState) { 
        return Container(height: 450, padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const Divider(), 
          Expanded(child: GridView.builder(gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 3, crossAxisSpacing: 15, mainAxisSpacing: 15, childAspectRatio: 2.0), itemCount: _proGradientPalette.length, itemBuilder: (context, index) { 
            List<Color> g = _proGradientPalette[index]; 
            return GestureDetector(onTap: () { saveState(); setState(() { bgGradient = g; bgImageBytes = null; }); setModalState((){}); Navigator.pop(context); }, child: Container(decoration: BoxDecoration(gradient: LinearGradient(colors: g), borderRadius: BorderRadius.circular(10), border: Border.all(color: Colors.grey.shade300, width: 1.5)))); 
          }))
        ])); 
      }); 
    }); 
    ...
    showModalBottomSheet(context: context, backgroundColor: Colors.white, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (context) { 
      return StatefulBuilder(builder: (context, setModalState) { 
        return Container(height: 550, padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('Gradient Tool (شیڈز)', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)), IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(context))]), 
          const Divider(),
          Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.grey.shade100, borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.grey.shade300)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Make Your Own (اپنی مرضی کا شیڈ بنائیں):', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)), const SizedBox(height: 10),
            Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
              ...
              ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF8B5CF6)), onPressed: () { if(sel.customGradColor1 != null && sel.customGradColor2 != null) { saveState(); setState(() => sel.textGradient = [sel.customGradColor1!, sel.customGradColor2!]); setModalState((){}); } }, child: const Text('Apply', style: TextStyle(color: Colors.white)))
            ])
          ])),
                      ...
                      builder: (context, constraints) {
                        ...
                        return Container(
                          ...
                          child: RepaintBoundary(
                            key: _canvasKey,
                            child: Container(
                              color: pageColor == Colors.transparent ? null : pageColor, 
                                    ...
                                    if (e.isShape) {
                                      ...
                                      Widget buildTextWidget(Color c, [List<Shadow>? shadow]) {
                                        return Text(
                                          ...
                                          style: TextStyle(fontFamily: e.fontFamily, fontSize: e.fontSize, letterSpacing: e.letterSpacing, color: c, fontWeight: e.isBold ? FontWeight.bold : FontWeight.normal, height: e.lineHeight, wordSpacing: e.wordSpacing, shadows: shadow),
                                        );
                                      }
                                      ...
                                      Widget txt = Stack(clipBehavior: Clip.none, children: blockLayers);

                                      if (e.hasStroke) {
                                        Widget strokeTxt = Text(e.content, textAlign: e.textAlign, softWrap: true, textDirection: TextDirection.rtl, style: TextStyle(fontFamily: e.fontFamily, fontSize: e.fontSize, letterSpacing: e.letterSpacing, foreground: Paint()..style = PaintingStyle.stroke..strokeWidth = e.strokeWidth..color = e.strokeColor, fontWeight: e.isBold ? FontWeight.bold : FontWeight.normal, height: e.lineHeight, wordSpacing: e.wordSpacing));
                                        ...
                                        onPanUpdate: (d) {
                                          if(!e.isLocked) {
                                            setState(() { 
                                              ...
                                              double eCenterY = e.y + (e.isText ? 100 : currentHeight) / 2 + 15;
                                              
                                              _snapV = (eCenterX - canvasW/2).abs() < 12;
                                              _snapH = (eCenterY - canvasH/2).abs() < 12;

                                              if (_snapV) e.x = canvasW/2 - currentWidth/2 - 20;
                                              if (_snapH) e.y = canvasH/2 - (e.isText ? 100 : currentHeight)/2 - 15;
                                            });
                                          }
                                        },
                                        child: Transform(
            ...
            child: SingleChildScrollView(
              ...
              child: Row(
                children: [
                  ...
                  // 🔥 NEW: BG COLOR & BG GRADIENT BUTTONS 🔥
          ...
          child: SingleChildScrollView(
            ...
            child: Row(
              children: [
                ...
                if (sel.imageBytes != null) _buildToolBtn(Icons.format_paint, 'Tint Color', () { saveState(); setState(() => sel.isTinted = !sel.isTinted); }),
