...
// ================= WORKSPACE ELEMENTS (MODELS STRICT TYPES) =================
class ProjectModel {
...
class _ProWorkspaceScreenState extends State<ProWorkspaceScreen> {
  ...
  Future<void> _captureAndSave(String format) async {
    ...
    try {
      ...
      if (format == 'JPG' || format == 'PNG') {
        final Map? result = await ImageGallerySaver.saveImage(pngBytes, quality: 100, name: "QalamKaarPro_${DateTime.now().millisecondsSinceEpoch}") as Map?;
  ...
  void _showColorPickerModal(DesignElement sel) { 
    ...
    Padding(padding: const EdgeInsets.only(bottom: 10), child: Row(children: [Expanded(child: TextField(controller: hexCtrl, decoration: const InputDecoration(hintText: 'Custom Hex: FFFF0000', border: OutlineInputBorder(), contentPadding: EdgeInsets.symmetric(horizontal: 10)))), const SizedBox(width: 10), ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF8B5CF6), padding: const EdgeInsets.symmetric(vertical: 12)), onPressed: () { String hex = hexCtrl.text.replaceAll('#', ''); if(hex.length == 6) hex = 'FF$hex'; if(hex.length == 8) { saveState(); setState((){ int colorInt = int.tryParse(hex, radix: 16) ?? 0xFFFFFFFF; if(sel.isText){ sel.textColor = Color(colorInt); sel.textGradient = null; } else { sel.elementColor = Color(colorInt); } }); setModalState((){}); } }, child: const Text('Apply', style: TextStyle(color: Colors.white)))])),
