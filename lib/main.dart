...
class DesignElement {
  ...
  // 🔥 NEW: Step 1 Advanced Text Features 🔥
  Color? textBgColor;
  double textBgRadius;
  double wordSpacing;
...
class _ProWorkspaceScreenState extends State<ProWorkspaceScreen> {
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 10),
            _buildExportOption(Icons.picture_as_pdf, 'Save as Print HD PDF', 'Zero Margins & Normal Size', Colors.red, () {
  ...
  void _showTextBgPickerModal(DesignElement sel) {
    List<Color> colorPalette = [
      Colors.transparent, Colors.black, Colors.white, Colors.grey.shade800,
      const Color(0xFFEF4444), const Color(0xFFF97316), const Color(0xFFF59E0B), const Color(0xFFEAB308),
      const Color(0xFF10B981), const Color(0xFF14B8A6), const Color(0xFF3B82F6), const Color(0xFF8B5CF6),
      const Color(0xFFD946EF), const Color(0xFFF43F5E), const Color(0xFFD4AF37),
    ];
    showModalBottomSheet(context: context, backgroundColor: Colors.white, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (context) { return StatefulBuilder(builder: (context, setModalState) { return Container(height: 380, padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('Text Background (بیک گراؤنڈ)', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)), IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(context))]), const Divider(), Expanded(child: GridView.builder(gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 6, crossAxisSpacing: 15, mainAxisSpacing: 15), itemCount: colorPalette.length, itemBuilder: (context, index) { Color c = colorPalette[index]; bool isSelected = sel.textBgColor?.value == c.value || (c == Colors.transparent && sel.textBgColor == null); return GestureDetector(onTap: () { saveState(); setState(() { sel.textBgColor = c == Colors.transparent ? null : c; }); setModalState((){}); }, child: Container(decoration: BoxDecoration(color: c, shape: BoxShape.circle, border: Border.all(color: Colors.grey.shade300, width: 1.5)), child: isSelected ? Icon(c == Colors.transparent ? Icons.block : Icons.check, color: c.computeLuminance() > 0.5 || c == Colors.transparent ? Colors.black : Colors.white, size: 20) : (c == Colors.transparent ? const Icon(Icons.block, color: Colors.grey) : null))); }))])); }); });
  }

  // 🔥 NEW: TEXT GRADIENT PICKER 🔥
  void _showGradientPickerModal(DesignElement sel) {
    List<List<Color>> gradientPalette = [
      [Colors.red, Colors.orange], [Colors.blue, Colors.purple],
      [const Color(0xFF10B981), const Color(0xFF14B8A6)], [const Color(0xFFD4AF37), const Color(0xFFFFF2CD), const Color(0xFFD4AF37)],
      [const Color(0xFFEC4899), const Color(0xFF8B5CF6)], [Colors.grey.shade800, Colors.black],
    ];
    showModalBottomSheet(context: context, backgroundColor: Colors.white, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (context) { return StatefulBuilder(builder: (context, setModalState) { return Container(height: 380, padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('Gradient Text (ملٹی کلر)', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)), IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(context))]), const Divider(), 
      ...
      Expanded(child: GridView.builder(gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 3, crossAxisSpacing: 15, mainAxisSpacing: 15, childAspectRatio: 2.0), itemCount: gradientPalette.length, itemBuilder: (context, index) { List<Color> g = gradientPalette[index]; return GestureDetector(onTap: () { saveState(); setState(() { sel.textGradient = g; }); setModalState((){}); Navigator.pop(context); }, child: Container(decoration: BoxDecoration(gradient: LinearGradient(colors: g), borderRadius: BorderRadius.circular(10), border: Border.all(color: Colors.grey.shade300, width: 1.5)))); }))])); }); });
  }
  ...
  void _showColorPickerModal(DesignElement sel) {
    ...
    showModalBottomSheet(context: context, backgroundColor: Colors.white, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (context) { return StatefulBuilder(builder: (context, setModalState) { Color currentColor = sel.isText ? sel.textColor : sel.elementColor; return Container(height: 380, padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('Color (رنگ منتخب کریں)', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)), IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(context))]), const Divider(), Expanded(child: GridView.builder(gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 6, crossAxisSpacing: 15, mainAxisSpacing: 15), itemCount: colorPalette.length, itemBuilder: (context, index) { Color c = colorPalette[index]; bool isSelected = currentColor.value == c.value; return GestureDetector(onTap: () { saveState(); setState(() { if (sel.isText) { sel.textColor = c; sel.textGradient = null; } else { sel.elementColor = c; } }); setModalState((){}); }, child: Container(decoration: BoxDecoration(color: c, shape: BoxShape.circle, border: Border.all(color: Colors.grey.shade300, width: 1.5), boxShadow: isSelected ? [BoxShadow(color: c.withOpacity(0.5), blurRadius: 8, spreadRadius: 2)] : null), child: isSelected ? Icon(Icons.check, color: c.computeLuminance() > 0.5 ? Colors.black : Colors.white, size: 20) : null)); }))])); }); });
  }
  ...
  // 🔥 NEW: COMBINED SPACING MODAL (LINE + WORD SPACING) 🔥
                    ...
                    children: [
                      ...
                      ...elements.map((e) {
                        ...
                        if (e.isShape) {
                          ...
                          // 🔥 APPLYING STEP 1 FEATURES (GRADIENT & SPACING) 🔥
                          Widget txt = Text(
                          ...
                          if (e.textGradient != null) {
                            txt = ShaderMask(
                              shaderCallback: (bounds) => LinearGradient(colors: e.textGradient!).createShader(Rect.fromLTWH(0, 0, bounds.width, bounds.height)),
                              child: txt,
                            );
                          }
                          ...
                          if (e.textBgColor != null) {
                            txt = Container(
                              ...
                              decoration: BoxDecoration(color: e.textBgColor, borderRadius: BorderRadius.circular(e.textBgRadius)),
                              child: txt,
                            );
                          }
        ...
        Container(
          ...
          child: SingleChildScrollView(
            ...
            child: Row(
              children: [
                const SizedBox(width: 10),
                if (sel.isText) _buildToolBtn(Icons.text_fields, 'Size', () => setState(() => sel.fontSize = sel.fontSize == 40 ? 60 : 40)),
                
                // 🔥 NEW: Spacing Button now opens multi-slider modal
                if (sel.isText) _buildToolBtn(Icons.height, 'Spacing', () => showSpacingModal(sel)),
                
                // 🔥 NEW: Text Background Button
