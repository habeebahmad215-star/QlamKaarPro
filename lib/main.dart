...
class DesignPage {
  ...
  double canvasRatio; // 🔥 NEW: Saved Canvas Ratio
  ...
  factory DesignPage.fromJson(Map<String, dynamic> json) => DesignPage(
    ...
    canvasRatio: json['canvasRatio'] ?? 0.707,
...
class DesignElement {
  ...
  int imageFilter; // 🔥 NEW: Image Filter Type (0=None, 1=Gray, 2=Sepia, 3=Invert)
  ...
  factory DesignElement.fromJson(Map<String, dynamic> json) => DesignElement(
    ...
    isLocked: json['isLocked'] ?? false, isHidden: json['isHidden'] ?? false, imageFilter: json['imageFilter'] ?? 0,
...
class _ProWorkspaceScreenState extends State<ProWorkspaceScreen> {
  ...
  // 🔥 NEW: Image Filter Matrices
  final List<ColorFilter> _filters = [
    const ColorFilter.mode(Colors.transparent, BlendMode.multiply), // 0: None
    const ColorFilter.matrix(<double>[0.2126, 0.7152, 0.0722, 0, 0, 0.2126, 0.7152, 0.0722, 0, 0, 0.2126, 0.7152, 0.0722, 0, 0, 0, 0, 0, 1, 0]), // 1: Gray
    const ColorFilter.matrix(<double>[0.393, 0.769, 0.189, 0, 0, 0.349, 0.686, 0.168, 0, 0, 0.272, 0.534, 0.131, 0, 0, 0, 0, 0, 1, 0]), // 2: Sepia
    const ColorFilter.matrix(<double>[-1, 0, 0, 0, 255, 0, -1, 0, 0, 255, 0, 0, -1, 0, 255, 0, 0, 0, 1, 0]), // 3: Invert
  ];
  ...
  // 🔥 NEW PRO FEATURE: Auto Alignment
  void _autoAlign(DesignElement sel, String alignType) {
    ...
    double eh = sel.isShape ? (sel.height > 20 ? sel.height : 90) : (sel.isText ? sel.fontSize * 1.5 : 150);
    saveState();
    setState(() {
  ...
  // 🔥 NEW PRO FEATURE: Canvas Resizer
            ...
            child: GestureDetector(
              ...
              child: Center(
                child: InteractiveViewer(
                  ...
                  child: AspectRatio(
                    aspectRatio: currentCanvasRatio, // 🔥 RATIO UPDATE APPLIES HERE
                    child: Container(
