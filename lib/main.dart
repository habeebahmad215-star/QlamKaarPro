...
class _ProWorkspaceScreenState extends State<ProWorkspaceScreen> {
  ...
  // ==== EXPORT & SAVE LOGIC (ULTRA HD & PERFECT FIT PDF) ====
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 10),
            _buildExportOption(Icons.picture_as_pdf, 'Save as HD PDF', 'Best for Printing (No Blur)', Colors.red, () {
  ...
  Future<void> _captureAndSave(String format) async {
    // 1. Hide Selection & Safe Area BEFORE capture
    setState(() {
      selectedId = null;
      _isExporting = true; 
    });
    
    // 2. Extra delay taaki UI fully refresh ho jaye aur Safe Area poori tarah hide ho
    await Future.delayed(const Duration(milliseconds: 500));
    
    try {
      RenderRepaintBoundary boundary = _canvasKey.currentContext!.findRenderObject() as RenderRepaintBoundary;
      
      // 3. ULTRA HD Logic: PDF ke liye 6.0 Zoom, Images ke liye 3.0 Zoom
      double pixelRatio = format == 'PDF' ? 6.0 : 3.0;
      ...
      if (format == 'JPG' || format == 'PNG') {
        ...
        // 4. FIX: "Page ke upar page" ko hatane ke liye exact canvas size ka page banaya
        double logicalWidth = image.width / pixelRatio;
        double logicalHeight = image.height / pixelRatio;

        pdf.addPage(
          pw.Page(
            pageFormat: PdfPageFormat(logicalWidth, logicalHeight),
            margin: pw.EdgeInsets.zero, // Zero Margin
            build: (pw.Context context) {
              return pw.Image(imagePdf, fit: pw.BoxFit.cover); // Image full page cover karegi
            },
          ),
        );
      ...
      // 5. Restore Safe Area AFTER capture
      setState(() {
        _isExporting = false; 
      });
    }
          ...
          child: AspectRatio(
            ...
            child: Container(
              ...
              child: RepaintBoundary(
                key: _canvasKey,
                child: Container(
                  color: pageColor, // Canvas background color
                  child: Stack(
                    ...
                    children: [
                      ...
                      ...elements.map((e) {
                        ...
                        // 🔴 Agar Export ho raha hai toh hum extra padding ya border nahi dikhayenge!
                        if (_isExporting) {
                          return Positioned(
                            left: e.x, top: e.y,
                            child: Opacity(opacity: e.opacity, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 25, vertical: 20), child: contentWidget)),
                          );
                        }
                        ...
                        return Positioned(
                          ...
                          child: GestureDetector(
                            ...
                            child: isSel 
                              ...
                              : Padding(padding: const EdgeInsets.symmetric(horizontal: 25, vertical: 20), child: Opacity(opacity: e.opacity, child: contentWidget)),
                          ),
                        );
                      }).toList(),
                    ],
