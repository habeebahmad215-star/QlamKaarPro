...
class _MyFolderScreenState extends State<MyFolderScreen> {
  ...
  Future<void> _loadProjects() async {
    ...
    setState(() {
      savedProjects = projStrings.map((s) => ProjectModel.fromJson(jsonDecode(s))).toList();
...
class ProjectModel {
  ...
  factory ProjectModel.fromJson(Map<String, dynamic> json) => ProjectModel(
    id: json['id'], name: json['name'], lastModified: json['lastModified'],
    pages: (json['pages'] as List).map((p) => DesignPage.fromJson(p)).toList(),
  );
}
...
class DesignPage {
  ...
  factory DesignPage.fromJson(Map<String, dynamic> json) => DesignPage(
    title: json['title'], pageColor: Color(json['pageColor']),
    elements: (json['elements'] as List).map((e) => DesignElement.fromJson(e)).toList(),
  );
}
...
class DesignElement {
  ...
  factory DesignElement.fromJson(Map<String, dynamic> json) => DesignElement(
    id: json['id'], x: json['x'], y: json['y'], content: json['content'],
    imageBytes: json['imageBytes'] != null ? base64Decode(json['imageBytes']) : null,
    isText: json['isText'] ?? true, fontSize: json['fontSize'] ?? 40.0, textColor: Color(json['textColor'] ?? 0xFF000000), opacity: json['opacity'] ?? 1.0, angle: json['angle'] ?? 0.0,
    isBold: json['isBold'] ?? false, isItalic: json['isItalic'] ?? false, textAlign: TextAlign.values[json['textAlign'] ?? 1], lineHeight: json['lineHeight'] ?? 1.5, fontFamily: json['fontFamily'] ?? 'JameelNoori',
    isBorder: json['isBorder'] ?? false, isShape: json['isShape'] ?? false, elementColor: Color(json['elementColor'] ?? 0xFFD4AF37), width: json['width'] ?? 0.0, height: json['height'] ?? 0.0,
    borderWidth: json['borderWidth'] ?? 5.0, borderStyle: json['borderStyle'] ?? 'royal_islamic',
    textBgColor: json['textBgColor'] != null ? Color(json['textBgColor']) : null, textBgRadius: json['textBgRadius'] ?? 10.0, wordSpacing: json['wordSpacing'] ?? 0.0,
    textGradient: json['textGradient'] != null ? (json['textGradient'] as List).map((c) => Color(c)).toList() : null,
    hasStroke: json['hasStroke'] ?? false, strokeColor: Color(json['strokeColor'] ?? 0xFFFFFFFF), strokeWidth: json['strokeWidth'] ?? 3.0,
    hasShadow: json['hasShadow'] ?? false, shadowColor: Color(json['shadowColor'] ?? 0x8A000000), shadowBlur: json['shadowBlur'] ?? 5.0, shadowOffsetX: json['shadowOffsetX'] ?? 3.0, shadowOffsetY: json['shadowOffsetY'] ?? 3.0,
  );
      ...
      appBar: AppBar(
        ...
        actions: [
          InkWell(
            ...
            child: Container(margin: const EdgeInsets.symmetric(vertical: 8), padding: const EdgeInsets.symmetric(horizontal: 10), decoration: BoxDecoration(border: Border.all(color: const Color(0xFF8B5CF6)), borderRadius: BorderRadius.circular(8)), alignment: Alignment.center, child: Row(children: const [Icon(Icons.save, color: Color(0xFF8B5CF6), size: 16), SizedBox(width: 5), Text('Save', style: TextStyle(color: Color(0xFF8B5CF6), fontWeight: FontWeight.bold))])),
          ),
          const SizedBox(width: 10),
          InkWell(
            onTap: _showExportMenu,
            child: Container(margin: const EdgeInsets.symmetric(vertical: 8), padding: const EdgeInsets.symmetric(horizontal: 10), decoration: BoxDecoration(color: const Color(0xFF8B5CF6), borderRadius: BorderRadius.circular(8)), alignment: Alignment.center, child: Row(children: const [Icon(Icons.download, color: Colors.white, size: 16), SizedBox(width: 5), Text('Export', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))])),
          ),
          const SizedBox(width: 10),
        ],
      ),
        ...
        children: [
          Positioned.fill(
            child: GestureDetector(
              ...
              child: Center(
                // 🔥 NEW: INTERACTIVE VIEWER WITH CONTROLLER 🔥
                child: InteractiveViewer(
                  transformationController: _transformController,
                  panEnabled: !_isCanvasLocked && !hasSelection, // Lock checks
                          ...
                          child: Stack(
                            ...
                            children: [
                              ...
                              ...elements.map((e) {
                                ...
                                if (_isExporting) { 
                                  return Positioned(left: e.x, top: e.y, child: Transform.rotate(angle: e.angle, alignment: Alignment.center, child: Opacity(opacity: e.opacity, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 15), child: contentWidget)))); 
                                }

                                // 🔥 ACTION BUTTONS SKELETON FIX 🔥
                                // Using Column Layout instead of Stack limits so clicks actually register!
                                return Positioned(
                                  left: e.x, top: e.y,
                                  child: GestureDetector(
                                    onTap: () => setState(() => selectedId = e.id),
                                              ...
                                              children: [
                                                Stack(
                                                  ...
                                                  children: [
                                                    ...
                                                    if (e.isText) Positioned(right: -15, top: 0, bottom: 0, child: GestureDetector(onPanUpdate: (d) { setState(() { double w = (e.width > 0 ? e.width : 280.0) + d.delta.dx; if (w > 50) e.width = w; }); }, child: Container(width: 30, color: Colors.transparent, alignment: Alignment.center, child: Container(width: 8, height: 25, decoration: BoxDecoration(color: const Color(0xFF8B5CF6), borderRadius: BorderRadius.circular(10)))))),
                                                    if (e.isText) Positioned(left: -15, top: 0, bottom: 0, child: GestureDetector(onPanUpdate: (d) { setState(() { double newW = (e.width > 0 ? e.width : 280.0) - d.delta.dx; if (newW > 50) { e.width = newW; e.x += d.delta.dx; } }); }, child: Container(width: 30, color: Colors.transparent, alignment: Alignment.center, child: Container(width: 8, height: 25, decoration: BoxDecoration(color: const Color(0xFF8B5CF6), borderRadius: BorderRadius.circular(10)))))),
                                                  ],
                                                ),
                                                const SizedBox(height: 15),
                                                // Working Action Buttons Array
                                                Material(
          ...
          // 🔥 NEW: FLOATING CANVAS CONTROLS (RESET ZOOM & LOCK) 🔥
          if (!_isExporting)
            Positioned(
              top: 20, right: 15,
              child: Column(
                children: [
                  InkWell(
                    ...
                    child: Container(
                      ...
                      child: Icon(_isCanvasLocked ? Icons.lock : Icons.lock_open, color: _isCanvasLocked ? Colors.redAccent : Colors.black87, size: 22),
                    ),
                  ),
                  const SizedBox(height: 12),
                  InkWell(
                    onTap: () {
                      _transformController.value = Matrix4.identity(); // Resets to original size
                    },
                    child: Container(
                      padding: const EdgeInsets.all(10),
                      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.1), blurRadius: 6)]),
                      child: const Icon(Icons.zoom_out_map, color: Colors.black87, size: 22),
                    ),
                  ),
                ],
              ),
