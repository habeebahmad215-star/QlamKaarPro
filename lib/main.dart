...
class _ProWorkspaceScreenState extends State<ProWorkspaceScreen> {
  ...
  // ==== EXPORT & SAVE LOGIC (PRINT-READY FIX 🔥) ====
  ...
  Future<void> _captureAndSave(String format) async {
    ...
    // UI hide hone ke liye 400ms ka waqt diya
    await Future.delayed(const Duration(milliseconds: 400));
    
    try {
      RenderRepaintBoundary boundary = _canvasKey.currentContext!.findRenderObject() as RenderRepaintBoundary;
      
      // 🚀 3.0 Pixel Ratio (300 DPI) - Print ke liye best hai aur file size 1-3 MB rakhega
      ...
      if (format == 'JPG' || format == 'PNG') {
        ...
        // 🔥 PERFECT PDF LOGIC (No Crash, No Margins, Normal Size) 🔥
        final pdf = pw.Document();
        final imagePdf = pw.MemoryImage(pngBytes);
        
        pdf.addPage(
          pw.Page(
            // Page ka size wahi hoga jo aapke canvas ka size hai (Edge-to-Edge)
            pageFormat: PdfPageFormat(image.width.toDouble(), image.height.toDouble()), 
            margin: pw.EdgeInsets.zero, // White border hamesha ke liye khatam!
