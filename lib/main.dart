...
// ================= WORKSPACE ELEMENTS (WITH JSON SUPPORT) =================
class ProjectModel {
...
class DesignElement {
  ...
  Map<String, dynamic> toJson() => {
    ...
    'isText': isText, 'fontSize': fontSize, 'textColor': textColor.value, 'opacity': opacity,
  ...
  factory DesignElement.fromJson(Map<String, dynamic> json) => DesignElement(
    ...
    isText: json['isText'] ?? true, fontSize: json['fontSize'] ?? 40.0, textColor: Color(json['textColor'] ?? 0xFF000000), opacity: json['opacity'] ?? 1.0,
...
class ProWorkspaceScreen extends StatefulWidget {
  final ProjectModel? project; // 🔥 FOR LOADING EXISTING PROJECTS
...
class _ProWorkspaceScreenState extends State<ProWorkspaceScreen> {
  ...
  Future<void> _saveProjectLocally() async {
    ...
    savedStrings.removeWhere((str) => jsonDecode(str)['id'] == projectId); // Remove old version
    savedStrings.add(jsonEncode(p.toJson())); // Add new version
  ...
  PdfColor _getPdfColor(Color color) { return PdfColor(color.red / 255.0, color.green / 255.0, color.blue / 255.0, color.alpha / 255.0); }
                  ...
                  child: Stack(
                    ...
                    children: [
                      ...
                      ...elements.map((e) {
                        ...
                        if (_isExporting) { return Positioned(left: e.x, top: e.y, child: Opacity(opacity: e.opacity, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 15), child: contentWidget))); }
