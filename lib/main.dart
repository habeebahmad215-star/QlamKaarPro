...
// 🔥 FIX FOR URDU CURVED TEXT (Words instead of single disconnected letters) 🔥
class CurvedTextWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Instead of breaking by single characters (which breaks Urdu joined letters),
    // we split by spaces to keep whole words connected.
    ...
    return SizedBox(
      width: radius.abs() * 2.5, height: radius.abs() * 2.5,
      child: Stack(
        ...
        children: List.generate(words.length, (index) {
          ...
          return Transform(
            ...
            alignment: Alignment.center,
            child: Text(words[index], style: style), // Renders the WHOLE word
          );
        }),
      ),
    );
...
class _ProWorkspaceScreenState extends State<ProWorkspaceScreen> {
  ...
  // 🔥 NEW PRO FEATURE MODALS: CURVE TEXT & BLEND MODES 🔥
