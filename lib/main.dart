...
class _ProWorkspaceScreenState extends State<ProWorkspaceScreen> {
  // 📸 Save karne ke liye global key (Canvas Capture)
  ...
  // ==== EXPORT & SAVE LOGIC (NAYA FEATURE) ====
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            // JPG Option
            _buildExportOption(Icons.image, 'Save as JPG', 'High Quality Image (White Background)', Colors.blue, () {
              Navigator.pop(context);
              _captureAndSave('JPG');
            }),
            const SizedBox(height: 10),
            // PNG Option
            _buildExportOption(Icons.layers_clear, 'Save as PNG', 'Transparent Image (For Logos)', Colors.purple, () {
              Navigator.pop(context);
              _captureAndSave('PNG');
            }),
            const SizedBox(height: 10),
            // PDF Option
            _buildExportOption(Icons.picture_as_pdf, 'Save as HD PDF', 'Best for Printing', Colors.red, () {
              Navigator.pop(context);
              _captureAndSave('PDF');
            }),
          ],
  ...
  Future<void> _captureAndSave(String format) async {
    // 1. Sabse pehle selection box (purple border) hatayein taaki wo image me na aaye
    setState(() => selectedId = null);
    
    // 2. Thoda wait karein taaki screen refresh ho jaye
    await Future.delayed(const Duration(milliseconds: 300));
    
    try {
      // 3. Canvas ka High Quality Screenshot lein
      RenderRepaintBoundary boundary = _canvasKey.currentContext!.findRenderObject() as RenderRepaintBoundary;
      ui.Image image = await boundary.toImage(pixelRatio: 3.0); // 3.0 means Ultra HD!
          ...
          builder: (context) => AlertDialog(
            ...
            content: Column(
              ...
              children: [
                ...
                const SizedBox(height: 15),
                Text('Design Captured!', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                const SizedBox(height: 10),
                Text('Format: $format\nSize: ${(pngBytes.lengthInBytes / 1024).toStringAsFixed(2)} KB', textAlign: TextAlign.center, style: const TextStyle(color: Colors.grey)),
                const SizedBox(height: 20),
                ElevatedButton(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          // 🔴 "SAVE PRO" BUTTON WALA EXPORT MENU 🔴
          InkWell(
      ...
      body: GestureDetector(
        ...
        child: Center(
          child: AspectRatio(
            ...
            // 📸 CANVAS KO REPAINT BOUNDARY MEIN DAAL DIYA TAAKI HD SAVE HO SAKE
