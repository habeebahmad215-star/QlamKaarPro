...
class DesignElement {
  ...
  // 🔥 NEW PRO FEATURES 🔥
  Uint8List? textTextureBytes; // For Text Masking (Sona/Chandi)
  double letterSpacing; // For precision text spacing
  ...
  DesignElement({
    ...
    this.textTextureBytes, this.letterSpacing = 0.0, // Initialized new features
  ...
  Map<String, dynamic> toJson() => {
    ...
    'textTextureBytes': textTextureBytes != null ? base64Encode(textTextureBytes!) : null,
...
// 🔥 NEW PRO FEATURE: TEXTURE MASK WRAPPER 🔥
...
class _TextureTextWrapperState extends State<TextureTextWrapper> {
  ...
  Widget build(BuildContext context) {
    ...
    return ShaderMask(
      blendMode: BlendMode.srcATop, // Fills only text area
      shaderCallback: (bounds) {
        final matrix = Matrix4.identity();
        // Scale texture to cover the text size
        matrix.scale(bounds.width / _image!.width, bounds.height / _image!.height);
...
class _ProWorkspaceScreenState extends State<ProWorkspaceScreen> {
  ...
  // 🔥 NEW PRO FEATURE: SNAP GUIDES STATE 🔥
  bool _snapV = false;
  bool _snapH = false;
  ...
  // 🔥 NEW PRO FEATURE: ADD TEXTURE FROM GALLERY 🔥
  ...
  // 🔥 ADVANCED SPACING WITH LETTER SPACING 🔥
                                ...
                                children: [
                                  ...
                                  ...elements.map((e) {
                                    ...
                                    if (e.isShape) {
                                      ...
                                      // 🔥 ADVANCED: LETTER SPACING APPLIED 🔥
                                      Widget txt = Text(
                                        e.content, textAlign: e.textAlign, softWrap: true, textDirection: TextDirection.rtl,
                                        style: TextStyle(fontFamily: e.fontFamily, fontSize: e.fontSize, letterSpacing: e.letterSpacing, color: e.textGradient != null ? Colors.white : e.textColor, fontWeight: e.isBold ? FontWeight.bold : FontWeight.normal, height: e.lineHeight, wordSpacing: e.wordSpacing, shadows: textShadows.isNotEmpty ? textShadows : null),
                                      );
                                      ...
                                      // 🔥 PRO FEATURE: TEXTURE MASK APPLIED 🔥
                                      if (e.textTextureBytes != null) {
                                        txt = TextureTextWrapper(textureBytes: e.textTextureBytes, child: txt);
                                      }
                                    ...
                                    return Positioned(
                                      ...
                                      child: GestureDetector(
                                        ...
                                        onPanEnd: (d) => setState(() { _snapV = false; _snapH = false; }), // Clear guides
                                        onPanUpdate: (d) {
                                          if(!e.isLocked) {
                                            setState(() { 
                                              selectedId = e.id; 
                                              ...
                                              // 🔥 PRO FEATURE: MAGNETIC SNAPPING 🔥
                                              double eCenterX = e.x + currentWidth / 2 + 20; // 20 is padding
                                              double eCenterY = e.y + (e.height > 0 ? e.height : 100) / 2 + 15;
                                              
                                              _snapV = (eCenterX - canvasW/2).abs() < 12;
                                              _snapH = (eCenterY - canvasH/2).abs() < 12;

                                              if (_snapV) e.x = canvasW/2 - currentWidth/2 - 20;
                                              if (_snapH) e.y = canvasH/2 - (e.height > 0 ? e.height : 100)/2 - 15;
                                            });
                                          }
                                        },
                                        child: Transform(
                                  ...
                                  // 🔥 PRO FEATURE: RENDER MAGNETIC GUIDES 🔥
                                  if (_snapV && !_isExporting) Positioned(left: canvasW/2, top: 0, bottom: 0, child: Container(width: 2, color: Colors.blueAccent)),
                                  if (_snapH && !_isExporting) Positioned(top: canvasH/2, left: 0, right: 0, child: Container(height: 2, color: Colors.blueAccent)),
                                  if ((_snapH || _snapV) && !_isExporting) Positioned(top: canvasH/2 - 10, left: canvasW/2 - 10, child: const Icon(Icons.add, color: Colors.red, size: 20)),
                                ],
                              ),
                            ),
                          ),
          ...
          child: SingleChildScrollView(
            ...
            child: Row(
              children: [
                ...
                // 🔥 NEW PRO FEATURE: TEXTURE BUTTON 🔥
