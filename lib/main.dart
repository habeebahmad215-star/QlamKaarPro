...
class DesignElement {
  ...
  // 🔥 NEW: 3D, Flip and Crop properties
  ...
  factory DesignElement.fromJson(Map<String, dynamic> json) => DesignElement(
    ...
    pitch: json['pitch'] ?? 0.0, yaw: json['yaw'] ?? 0.0, isCircleCrop: json['isCircleCrop'] ?? false, flipX: json['flipX'] ?? false, flipY: json['flipY'] ?? false,
...
class _ProWorkspaceScreenState extends State<ProWorkspaceScreen> {
  ...
  Future<void> addImageFromGallery() async { try { final XFile? image = await _picker.pickImage(source: ImageSource.gallery); if (image != null) { final bytes = await image.readAsBytes(); saveState(); setState(() => elements.add(DesignElement(id: Random().nextInt(10000).toString(), x: 80, y: 80, content: '', imageBytes: bytes, isText: false, width: 250))); } } catch (e) {} Navigator.pop(context); }
    ...
    showModalBottomSheet(context: context, backgroundColor: Colors.white, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (context) { 
      return StatefulBuilder(builder: (context, setModalState) { 
        return Container(height: 280, padding: const EdgeInsets.all(20), child: Column(children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('3D Rotate (تھری ڈی زاویہ)', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)), IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(context))]), 
          ...
          ElevatedButton(onPressed: () { saveState(); setState((){ sel.pitch=0; sel.yaw=0; }); setModalState((){}); }, style: ElevatedButton.styleFrom(backgroundColor: Colors.grey.shade200), child: const Text('Reset 3D', style: TextStyle(color: Colors.black)))
        ])); 
      }); 
    });
  }
                          ...
                          child: Stack(
                            ...
                            children: [
                              ...
                              ...elements.map((e) {
                                ...
                                // 🔥 NEW: 3D MATRIX FOR ALL ELEMENTS 🔥
                                ...
                                double currentWidth = e.width > 50 ? e.width : 280.0; // Standardize Width
                                ...
                                if (e.isShape) {
                                  ...
                                  // 🔥 PRO FEATURE: IMAGE CROP & SIZING 🔥
                                  Widget img = Image.memory(e.imageBytes!, width: currentWidth, fit: BoxFit.contain);
                                  if (e.isCircleCrop) {
                                    contentWidget = Container(width: currentWidth, height: currentWidth, clipBehavior: Clip.antiAlias, decoration: const BoxDecoration(shape: BoxShape.circle), child: Image.memory(e.imageBytes!, fit: BoxFit.cover));
                                  } else {
          ...
          child: SingleChildScrollView(
            ...
            child: Row(
              children: [
                ...
                _buildToolBtn(Icons.view_in_ar, '3D Rotate', () => show3DModal(sel)), // 3D Modal 
