...
// Build Error Fix: Added 'covariant' keyword to comply with strict Dart 3 rules
...
class StarClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) { 
    ...
    for (int i = 0; i < 10; i++) { 
      ...
      double a = -pi/2 + step * i; 
      path.lineTo(halfWidth + r * cos(a), halfHeight + r * sin(a)); 
    } 
    path.close(); 
    return path; 
  }
...
// Build Error Fix: Using Matrix4 scaling safely to avoid compilation issues on Dart 3+
class TextureTextWrapper extends StatefulWidget {
  ...
  @override 
  State<TextureTextWrapper> createState() => _TextureTextWrapperState();
}
...
class _TextureTextWrapperState extends State<TextureTextWrapper> {
  ...
  @override 
  void didUpdateWidget(TextureTextWrapper oldWidget) { 
    super.didUpdateWidget(oldWidget); 
    if (widget.textureBytes != oldWidget.textureBytes) _loadImage(); 
  }
  
  Future<void> _loadImage() async { 
    if (widget.textureBytes == null) { 
      if (mounted) setState(() => _image = null); 
      return; 
    } 
    final codec = await ui.instantiateImageCodec(widget.textureBytes!); 
  ...
  @override 
  Widget build(BuildContext context) { 
    if (_image == null || widget.textureBytes == null) return widget.child; 
    return ShaderMask(
      blendMode: BlendMode.srcATop, 
      shaderCallback: (bounds) { 
        // Build Fix: Explicitly defining X, Y, Z for scale
        final matrix = Matrix4.identity(); 
        matrix.scale(bounds.width / _image!.width, bounds.height / _image!.height, 1.0); 
        return ImageShader(_image!, TileMode.clamp, TileMode.clamp, matrix.storage); 
      }, 
      child: widget.child
    ); 
  }
