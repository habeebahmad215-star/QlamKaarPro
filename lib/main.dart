...
class DesignElement {
  ...
  // 🔥 NEW PRO FEATURES: 3D TEXT BLOCK 🔥
  double text3dDepth; 
  Color text3dColor;
  ...
  DesignElement({
    ...
    this.text3dDepth = 0.0, this.text3dColor = Colors.black54, // Initialized 3D Block
...
class _ProWorkspaceScreenState extends State<ProWorkspaceScreen> {
  ...
  // 🔥 PRO FEATURE: 3D BLOCK TEXT MODAL 🔥
  void _show3DBlockModal(DesignElement sel) { 
    showModalBottomSheet(context: context, backgroundColor: Colors.white, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (context) { 
      return StatefulBuilder(builder: (context, setModalState) { 
        return Container(height: 480, padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Expanded(child: GridView.builder(gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 6, crossAxisSpacing: 10, mainAxisSpacing: 10), itemCount: _proColorPalette.length, itemBuilder: (context, index) { 
            Color c = _proColorPalette[index]; bool isSel = sel.text3dColor.value == c.value; 
            return GestureDetector(onTap: () { saveState(); setState(() => sel.text3dColor = c); setModalState((){}); }, child: Container(decoration: BoxDecoration(color: c, shape: BoxShape.circle, border: Border.all(color: Colors.grey.shade300, width: 1.5), boxShadow: isSel ? [BoxShadow(color: c.withOpacity(0.5), blurRadius: 6)] : null), child: isSel ? Icon(Icons.check, color: c.computeLuminance() > 0.5 ? Colors.black : Colors.white, size: 20) : null)); 
          }))
        ])); 
      }); 
    }); 
                                  ...
                                  ...elements.map((e) {
                                    ...
                                    if (e.isShape) {
                                      ...
                                      contentWidget = Container(
                                        width: currentWidth, 
                                        height: e.isCircleCrop ? currentWidth : currentHeight, 
                                        clipBehavior: Clip.antiAlias, 
                                        decoration: BoxDecoration(shape: e.isCircleCrop ? BoxShape.circle : BoxShape.rectangle, borderRadius: e.isCircleCrop ? null : BorderRadius.circular(e.cornerRadius), boxShadow: boxShadows.isNotEmpty ? boxShadows : null, border: universalBorder), 
                                        child: img
                                      );
                                    } else {
                                      ...
                                      // 🔥 NEW PRO FEATURE: REAL 3D TEXT EXTRUSION 🔥
                                                  ...
                                                  children: [
                                                    Stack(
                                                      ...
                                                      children: [
                                                        ...
                                                        // 🔥 PRO FEATURE: FREEHAND RESIZE HANDLES 🔥
                                                        // Right Handle (Width)
                                                        ...
                                                        // Bottom Handle (Height - Only for Images/Shapes)
                                                        ...
                                                        // Corner Handle (Proportional Scale)
          ...
          child: SingleChildScrollView(
            ...
            child: Row(
              children: [
                ...
                if (sel.imageBytes != null) _buildToolBtn(Icons.photo_filter, 'Filters', () => _showImageFiltersModal(sel)),
