...
class _ProWorkspaceScreenState extends State<ProWorkspaceScreen> {
  ...
  // ==== EXPORT & SAVE TO GALLERY (NAYA FEATURE) ====
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 10),
            // PDF Option
            _buildExportOption(Icons.picture_as_pdf, 'Save as HD PDF', 'Best for Printing (Jald aa raha hai)', Colors.grey, () {
              Navigator.pop(context);
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('PDF feature jald aa raha hai! Abhi JPG try karein.')));
            }),
          ],
        ),
      ),
  ...
  // HD Screenshot & Save Engine
  Future<void> _captureAndSave(String format) async {
    // 1. Sabse pehle selection box hatayein taaki box save na ho jaye
    ...
    try {
      // 3. Canvas capture (3.0 = HD Quality)
      RenderRepaintBoundary boundary = _canvasKey.currentContext!.findRenderObject() as RenderRepaintBoundary;
      ui.Image image = await boundary.toImage(pixelRatio: 3.0);
      ByteData? byteData = await image.toByteData(format: ui.ImageByteFormat.png);
      Uint8List pngBytes = byteData!.buffer.asUint8List();

      // 4. Save to Mobile Gallery!
          ...
          builder: (context) => AlertDialog(
            ...
            content: Column(
              ...
              children: [
                ...
                const SizedBox(height: 15),
                const Text('Saved to Gallery!', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                const SizedBox(height: 10),
                Text('Aapka $format design phone ki gallery (Photos) mein save ho gaya hai.', textAlign: TextAlign.center, style: const TextStyle(color: Colors.grey)),
                const SizedBox(height: 20),
                ElevatedButton(
