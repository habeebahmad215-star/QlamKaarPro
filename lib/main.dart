...
class DesignElement {
  ...
  bool isPath; List<Offset>? drawPath;
...
class _ProWorkspaceScreenState extends State<ProWorkspaceScreen> {
  ...
  // 🔥 YAHAN HAIN MISSING VARIABLES 🔥 (Inhe galti se delete kar diya gaya tha!)
  ...
  final List<ColorFilter> _filters = [
    ...
    const ColorFilter.matrix(<double>[0.393, 0.769, 0.189, 0, 0, 0.349, 0.686, 0.168, 0, 0, 0.272, 0.534, 0.131, 0, 0, 0, 0, 0, 1, 0]), 
  ...
  Future<void> _captureAndSave(String format) async {
    setState(() { selectedId = null; _isExporting = true; _isDrawingMode = false; });
  ...
  Future<void> addImageFromGallery() async { try { final XFile? image = await _picker.pickImage(source: ImageSource.gallery); if (image != null) { final bytes = await image.readAsBytes(); saveState(); if (mounted) setState(() => elements.add(DesignElement(id: Random().nextInt(10000).toString(), x: 80, y: 80, content: '', imageBytes: bytes, isText: false, width: 250))); } } catch (e) { debugPrint(e.toString()); } if (mounted) Navigator.pop(context); }
  Future<void> _pickBgImage() async { try { final XFile? image = await _picker.pickImage(source: ImageSource.gallery); if (image != null) { final bytes = await image.readAsBytes(); saveState(); if (mounted) setState(() => bgImage = bytes); } } catch (e) { debugPrint(e.toString()); } }
  ...
  void _showColorPickerModal(DesignElement sel) { 
    showModalBottomSheet(context: context, isScrollControlled: true, backgroundColor: Colors.white, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (context) { 
      ...
      return StatefulBuilder(builder: (context, setModalState) { Color currentColor = sel.isText ? sel.textColor : sel.elementColor; return Padding(padding: EdgeInsets.only(bottom: MediaQuery.of(context).viewInsets.bottom), child: Container(height: 480, padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('Color (رنگ منتخب کریں)', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)), IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(context))]), 
  ...
  void showAlignSnapModal(DesignElement sel) {
    showModalBottomSheet(context: context, backgroundColor: Colors.white, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (context) { return Container(height: 250, padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('Smart Align (جادوئی سیدھ)', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)), IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(context))]), const Divider(), Expanded(child: GridView.count(crossAxisCount: 3, crossAxisSpacing: 10, mainAxisSpacing: 10, childAspectRatio: 2.0, children: [ ElevatedButton.icon(onPressed: () { _autoAlign(sel, 'center_h'); Navigator.pop(context); }, icon: const Icon(Icons.align_horizontal_center, size: 16), label: const Text('Center H', style: TextStyle(fontSize: 12)), style: ElevatedButton.styleFrom(backgroundColor: Colors.blue.shade50, foregroundColor: Colors.blue)), ElevatedButton.icon(onPressed: () { _autoAlign(sel, 'center_v'); Navigator.pop(context); }, icon: const Icon(Icons.align_vertical_center, size: 16), label: const Text('Center V', style: TextStyle(fontSize: 12)), style: ElevatedButton.styleFrom(backgroundColor: Colors.blue.shade50, foregroundColor: Colors.blue)), ElevatedButton.icon(onPressed: () { _autoAlign(sel, 'center_h'); _autoAlign(sel, 'center_v'); Navigator.pop(context); }, icon: const Icon(Icons.center_focus_strong, size: 16), label: const Text('Middle', style: TextStyle(fontSize: 12)), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF8B5CF6).withOpacity(0.1), foregroundColor: const Color(0xFF8B5CF6))), ElevatedButton.icon(onPressed: () { _autoAlign(sel, 'left'); Navigator.pop(context); }, icon: const Icon(Icons.align_horizontal_left, size: 16), label: const Text('Left', style: TextStyle(fontSize: 12)), style: ElevatedButton.styleFrom(backgroundColor: Colors.grey.shade100, foregroundColor: Colors.black87)), ElevatedButton.icon(onPressed: () { _autoAlign(sel, 'right'); Navigator.pop(context); }, icon: const Icon(Icons.align_horizontal_right, size: 16), label: const Text('Right', style: TextStyle(fontSize: 12)), style: ElevatedButton.styleFrom(backgroundColor: Colors.grey.shade100, foregroundColor: Colors.black87)), ElevatedButton.icon(onPressed: () { _autoAlign(sel, 'top'); Navigator.pop(context); }, icon: const Icon(Icons.align_vertical_top, size: 16), label: const Text('Top', style: TextStyle(fontSize: 12)), style: ElevatedButton.styleFrom(backgroundColor: Colors.grey.shade100, foregroundColor: Colors.black87)), ])) ])); });
  }
                            ...
                            children: [
                              ...
                              ...elements.map((e) {
                                ...
                                if (e.isPath && e.drawPath != null) {
                                  currentWidth = e.width > 10 ? e.width : 50;
                                  contentWidget = SizedBox(
                                    width: currentWidth,
                                    height: e.height > 10 ? e.height : 50,
                                    child: CustomPaint(
                                      painter: DrawingPainter(path: e.drawPath!, color: e.elementColor, width: e.strokeWidth, isLive: false),
                                    ),
                                  );
                                }
                                else if (e.isShape) {
                                              ...
                                              children: [
                                                Stack(
                                                  ...
                                                  children: [
                                                    ...
                                                    Positioned(right: -15, top: 0, bottom: 0, child: GestureDetector(onPanUpdate: (d) { setState(() { double w = currentWidth + d.delta.dx; if (w > 20) e.width = w; }); }, child: Container(width: 30, color: Colors.transparent, alignment: Alignment.center, child: Container(width: 8, height: 25, decoration: BoxDecoration(color: const Color(0xFF8B5CF6), borderRadius: BorderRadius.circular(10)))))),
                                                    Positioned(left: -15, top: 0, bottom: 0, child: GestureDetector(onPanUpdate: (d) { setState(() { double newW = currentWidth - d.delta.dx; if (newW > 20) { e.width = newW; e.x += d.delta.dx; } }); }, child: Container(width: 30, color: Colors.transparent, alignment: Alignment.center, child: Container(width: 8, height: 25, decoration: BoxDecoration(color: const Color(0xFF8B5CF6), borderRadius: BorderRadius.circular(10)))))),
                                                  ],
                                                ),
                                                const SizedBox(height: 15),
                                                Material(
                              ...
                              // 🔥 YAHAN HAI WOH SAFE REDUCE FORMULA FIX 🔥
                              if (_isDrawingMode)
                                Positioned.fill(
                                  child: GestureDetector(
                                    onPanStart: (d) {
                                    ...
                                    onPanUpdate: (d) {
                                      setState(() {
                                        _currentPath = List.from(_currentPath)..add(d.localPosition);
                                      });
                                    },
                                    onPanEnd: (d) {
                                      if (_currentPath.length > 1) {
                                        double minX = _currentPath.map((o) => o.dx).reduce((a, b) => a < b ? a : b);
                                        double maxX = _currentPath.map((o) => o.dx).reduce((a, b) => a > b ? a : b);
                                        double minY = _currentPath.map((o) => o.dy).reduce((a, b) => a < b ? a : b);
                                        double maxY = _currentPath.map((o) => o.dy).reduce((a, b) => a > b ? a : b);
                                        
                                        double pathW = maxX - minX;
                                        if (pathW < 20.0) pathW = 20.0;
                                        double pathH = maxY - minY;
                                        if (pathH < 20.0) pathH = 20.0;
                                    ...
                                    child: Container(
                                      ...
                                      child: CustomPaint(
                                        painter: DrawingPainter(path: _currentPath, color: _brushColor, width: _brushSize, isLive: true),
                                      ),
                                    )
                                  )
                                ),
        ...
        children: [
          ...
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Row(
                children: [
                  _buildToolBtn(Icons.brush, 'Draw', () => setState(() { _isDrawingMode = true; selectedId = null; })), 
  ...
  Widget _buildDrawingToolBar() {
    return Padding(
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          IconButton(icon: const Icon(Icons.close, color: Colors.red, size: 30), onPressed: () => setState(() { _isDrawingMode = false; _currentPath.clear(); })),
...
class DrawingPainter extends CustomPainter {
  ...
  DrawingPainter({required this.path, required this.color, required this.width, this.isLive = false});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (!isLive) {
      ...
      scaleX = size.width / maxX;
      scaleY = size.height / maxY;
    }

    final paint = Paint()
      ..color = color
      ..strokeWidth = isLive ? width : (width * min(scaleX, scaleY))
      ..strokeCap = StrokeCap.round
      ..strokeJoin = StrokeJoin.round
      ..style = PaintingStyle.stroke;

    final p = Path();
    p.moveTo(path[0].dx * scaleX, path[0].dy * scaleY);
    for (int i = 1; i < path.length; i++) {
      p.lineTo(path[i].dx * scaleX, path[i].dy * scaleY);
    }
